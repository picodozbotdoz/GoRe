...
func TestUpstreamServeHTTP(t *testing.T) {
	...
	upstream := NewUpstream("test", servers, "round-robin", nil, nil, true, 0, 0, "", "", 0, 0, nil, nil)
...
func TestProxyRedirect(t *testing.T) {
	...
	servers := []*Server{{Addr: backend.Listener.Addr().String(), Weight: 1}}
	boolTrue := true
	upstream := NewUpstream("test", servers, "round-robin", nil, nil, true, 0, 0, "old.example.com new.example.com", "", 0, 0, &boolTrue, &boolTrue)
...
func TestProxyPassRequestHeadersDisabled(t *testing.T) {
	...
	servers := []*Server{{Addr: backend.Listener.Addr().String(), Weight: 1}}
	boolFalse := false
	upstream := NewUpstream("test", servers, "round-robin", nil, map[string]string{"X-Custom": "value"}, true, 0, 0, "", "", 0, 0, &boolFalse, nil)
...
func TestProxyPassRequestBodyDisabled(t *testing.T) {
	...
	servers := []*Server{{Addr: backend.Listener.Addr().String(), Weight: 1}}
	boolFalse := false
	upstream := NewUpstream("test", servers, "round-robin", nil, nil, true, 0, 0, "", "", 0, 0, nil, &boolFalse)
